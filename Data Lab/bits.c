...
int getBit(int x, int p){
   // function throws away all bits on the left, so that bit on 'p' position is right-most.
   // bit-wise operation '&' with integer '1' is used, so that only the right-most bit can have a value of 1 or 0 (depends), and all the other bits are always 0.
   // returns 1 if p is 1, else returns 0
   int shiftx, bitp;
   shiftx = x >> p;
   bitp = shiftx & 1;
   return bitp;
 ...
 */
int anyOddBit(int x) {
   // performs right shift for comparison of 8 bits at a time
   // bitwise AND with mask ensures that only odd numbered bits can be 1
   // if no odd bits, then returns 0, else 1
   int bw1,bw2,bw3,bw4;
   bw1 = ((x >> 24) & 0xAA);
   bw2 = ((x >> 16) & 0xAA);
   bw3 = ((x >> 8) & 0xAA);
...
int patternMatching(short x, int p){
   int allpatterns;
   allpatterns = !((x & 0xF) ^ p) + !(((x >> 1) & 0xF)^p) + !(((x >> 2) & 0xF)^p) + !(((x >> 3) & 0xF)^p) + !(((x >> 4) & 0xF)^p) + !(((x >> 5) & 0xF)^p) + !(((x >> 6) & 0xF)^p) + !(((x >> 7) & 0xF) ^p) + !(((x >> 8) & 0xF)^p) + !(((x >> 9) & 0xF)^p) + !(((x >> 10) & 0xF)^p) + !(((x >> 11) & 0xF)^p);
   return allpatterns;
}
/* 
 * negate - return -x 
